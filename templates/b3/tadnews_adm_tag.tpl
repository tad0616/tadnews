    ...
    <form action="tag.php" method="post" id="myForm" class="form-horizontal" role="form">
        ...
        <{foreach item=tag from=$tagarr}>
            ...
            <tr>
                ...
                <td>
                    <a href='tag.php?tag_sn=<{$tag.tag_sn}>' class='btn btn-info'><{$smarty.const._TADNEWS_EDIT}></a>
                    <{if $tag.enable}>
                        <a href='tag.php?op=stat&enable=0&tag_sn=<{$tag.tag_sn}>' class='btn btn-warning'><{$smarty.const._MA_TADNEWS_TAG_UNABLE}></a>
                    <{else}>
                        <a href='tag.php?op=stat&enable=1&tag_sn=<{$tag.tag_sn}>' class='btn btn-success'><{$smarty.const._MA_TADNEWS_TAG_ABLE}></a>
                    <{/if}>
                    <{if $tag.enable!= '1' and $tag.tag_amount > 0}>
                        <a href='javascript:delete_tag(<{$tag.tag_sn}>);' class='btn btn-danger'><{$smarty.const._TADNEWS_DEL}></a>
                    <{/if}>
                </td>
            </tr>
            <{/if}>
