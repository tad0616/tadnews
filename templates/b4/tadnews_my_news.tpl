    ...
    <tbody>
      <{foreach item=page from=$page}>
        <tr>
          ...
          <td>
            <a href="javascript:delete_tad_news_func(<{$page.nsn}>);" class='btn btn-danger btn-sm'><{$smarty.const._TAD_DEL}></a>
            <a href="post.php?op=tad_news_form&nsn=<{$page.nsn}>" class="btn btn-warning btn-sm"><{$smarty.const._TAD_EDIT}></a>
            <{if $page.enable!='1'}>
              <a href="post.php?op=enable_news&nsn=<{$page.nsn}>" class="btn btn-success btn-sm"><{$smarty.const._MD_TADNEWS_NEWS_ENABLE_OK}></a>
            <{/if}>
          </td>
        </tr>
      <{/foreach}>
