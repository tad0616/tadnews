...
<{foreach from=$all_news item=news}>
    <{if $smarty.session.tadnews_adm or $news.ncsn|in_array:$ok_cat}>
    ...
    <ul class="list-group" id="sort_<{$news.ncsn}>" style="margin: 4px auto 30px;">
        <{foreach from=$news.news item=news}>
            <li class="list-group-item d-flex justify-content-between align-items-center" <{if $smarty.session.tadnews_adm or $news.ncsn|in_array:$ok_cat}>id="tr_<{$news.nsn}>"<{/if}>>
    ...
    <{if $news.ncsn|in_array:$link_cate_sn_arr and ($smarty.session.tadnews_adm or $news.ncsn|in_array:$ok_cat)}>
