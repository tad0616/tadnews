<script type="text/javascript">
  ...
  function view_content<{$block.randStr}>(p){
    $.post("<{$block.HTTP_HOST}>/modules/tadnews/ajax.php", { num: "<{$block.num}>", p: p , show_button:"<{$block.show_button}>",  "cell[]": ["<{$block.cell1}>", "<{$block.cell2}>", "<{$block.cell3}>", "<{$block.cell4}>", "<{$block.cell5}>"],start_from:"<{$block.start_from}>",show_ncsn: "<{$block.show_ncsn}>",randStr:"<{$block.randStr}>", ncsn:$("#ncsn<{$block.randStr}>").val(), tag_sn:$("#tag_sn<{$block.randStr}>").val(), keyword:$("#keyword<{$block.randStr}>").val(), start_day:$("#start_day<{$block.randStr}>").val(), end_day:$("#end_day<{$block.randStr}>").val()},
      function(data) {
