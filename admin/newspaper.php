...
$xoopsOption['template_main'] = "tadnews_adm_newspaper_tpl.html";
/*-----------function區--------------*/
...
$content=explode(_SEPARTE2,$news_content);
      ...
      $html.="<h3 class='TadNewsPaper_title'>"._MA_TADNEWS_NP_TITLE_L."{$cates[$ncsn]}"._MA_TADNEWS_NP_TITLE_R."<a href='".XOOPS_URL."/modules/tadnews/index.php?nsn={$nsn}' target='_blank'>{$news_title}</a></h3>
...
function sendmail_form($npsn=""){
  ...
  $i=1;
    ...
    $data=empty($mailData[$email])?_MD_TADNEWS_NEVER_SEND." <a href=\"javascript:delete_tad_news_email_func('$email');\" class='btn btn-mini btn-danger'>"._TADNEWS_DEL."</a>":$mailData[$email];
  ...
  $xoopsTpl->assign("total" , sprintf(_MD_TADNEWS_MAIL_LIST,$total)) ;
...
function newspaper_email($nps_sn=""){
  ...
  $xoopsTpl->assign("back" , sprintf(_MD_TADNEWS_BACK_TO,$np['title'])) ;
...
function sendmail_log($npsn=""){
 global $xoopsDB,$xoopsModule,$xoopsUser,$xoopsConfig,$xoopsTpl;
  ...
  $xoopsTpl->assign("nps_sn" , $newspaper['nps_sn']) ;
  $xoopsTpl->assign("npsn" , $npsn) ;
  $xoopsTpl->assign("back" , sprintf(_MD_TADNEWS_BACK_TO,$newspaper_title)) ;
